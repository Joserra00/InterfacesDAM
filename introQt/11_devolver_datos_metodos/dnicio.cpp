...
void DInicio::slotCambiarTexto(){
	...
	float cantidad;
	bool aceptado;
	aceptado = dTransferencia->getInfo(cuenta,cantidad);
	


	QString number=QString::number(dTransf->getCantidad());

	label->setText("Transf de: "+dTransf->getCuenta()+" eurillos: "+number);
	delete dTransf;
	dTransf = NULL;
